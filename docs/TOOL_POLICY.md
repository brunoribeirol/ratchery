...
machine-readable in `tools.lock.json`. No optional external tool is `always`:
...
aggregate `benchmark suites/capture/compare/report` workflow in
`BENCHMARKING.md`; use `budget check` only as a read-only estimated threshold,
not a billing boundary. Cost is estimated, and a vendor's claimed savings are
never Ratchetry evidence.
